...
Let's add pca recipe to our pre-processing recipe (don't forget to prep)!
...
```{r}
beans_baked_PCA <- beans_prepped_PCA |> 
  bake(new_data = NULL)
...
beans_prepped_ICA <- beans_rec_ICA |> 
  prep()

beans_baked_ICA <- beans_prepped_ICA |> 
  bake(new_data = NULL)
```
...
```{r}
beans_baked_ICA |> 
  ggplot(aes(IC4, IC5, label = class)) +
  geom_point(aes(color = class), alpha = 0.7, size = 2) +
  labs(color = NULL) +
  theme_minimal_grid()
```
